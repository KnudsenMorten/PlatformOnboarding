...
<#
.SYNOPSIS
    Community cloud-native launcher for PlatformOnboarding\New-EntraApp.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'New-EntraApp (community cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\PlatformOnboarding\SCRIPTS\New-EntraApp.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'New-EntraApp (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "New-EntraApp FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
