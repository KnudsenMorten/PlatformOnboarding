...
<#
.SYNOPSIS
    Community cloud-native launcher for PlatformOnboarding\Enable-VmManagedIdentity.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'Enable-VmManagedIdentity (community cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\PlatformOnboarding\SCRIPTS\Enable-VmManagedIdentity.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'Enable-VmManagedIdentity (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "Enable-VmManagedIdentity FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
